...
class Eggstar extends API
{
    ...
    protected function users()
    {
        ...
        return array(array('returnResponse' => 0));
    }
    ...
    protected function rights()
    {
        if($this->method == 'GET')
        {
            //cas de demande des informations de partage sur un élément spécifique
            if(isset($this->request['idUser']) && isset($this->request['idElement']))
                $shares = findSharesForElement($this->request['idElement'], $this->request['idUser']);

            if(array_key_exists('error', $shares))
                return array($shares);
            else
                return $shares;
        }
